...
if 'google.generativeai' not in sys.modules:
    st.info("Installing required packages. This might take a moment...")
...
if st.button("Generate Evaluation"):
    if not video_title or not video_description:
        ...
        try:
            ...
            # Note: Despite the public name "Gemini 1.5 Pro", the API uses "gemini-1.5-pro" as the model identifier
            model = genai.GenerativeModel('gemini-1.5-pro')
            
            # Generate content with temperature similar to the OpenAI example
            response = model.generate_content(
                prompt,
                generation_config=genai.types.GenerationConfig(
                    temperature=0.5,  # Same temperature as the OpenAI example
                    top_p=0.95,
                    top_k=40,
...
st.sidebar.markdown(
    "Provide the video title and description, then click Generate. The app will use Google's Gemini to produce a detailed evaluation."
)
